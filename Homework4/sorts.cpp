...
{
    // DONE: Using the insertion sort algorithm (pp. 311-313), sort s
    //       according to the ordering relationship passed in as the
    //       parameter comp.
    
    for (int unsorted = 1; unsorted < s.size(); unsorted ++)
    {
        Sensor nextSensor = s[unsorted];
        int loc = unsorted;
        while ( loc > 0 && comp(nextSensor, s[loc-1]) )
        {
            s[loc] = s[loc-1];
            loc --;
        }
